...
export default function FindPageClient({ listings, cities }: Props) {
  ...
  return (
    <div className="find-page" style={{ margin: '0 -24px', width: 'calc(100% + 48px)' }}>
      <div className="flex flex-col" style={{ height: 'calc(100vh - 80px)' }}>
        ...
        <div className="border-b border-pm-border bg-pm-bg px-4 py-3 shrink-0 z-20">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2.5 text-sm border border-pm-border rounded-xl bg-white focus:outline-none focus:border-pm-accent focus:ring-1 focus:ring-pm-accent/20 placeholder:text-pm-ash transition-colors"
              />
              {search && (
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2.5 text-sm border rounded-xl transition-colors shrink-0 ${
                hasActiveFilters ? 'border-pm-accent bg-pm-accent/5 text-pm-accent' : 'border-pm-border bg-white text-pm-muted hover:border-pm-accent/40'
              }`}
            >
              ...
              {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-pm-accent" />}
            </button>

            <div className="hidden md:flex items-center text-xs text-pm-faint shrink-0">
              <span className="font-medium text-pm-muted">{venueCount}</span>
              <span className="ml-1">venue{venueCount !== 1 ? 's' : ''}</span>
              {courtCount > 0 && (
                <><span className="mx-1.5 text-pm-ash">·</span><span className="font-medium text-pm-muted">{courtCount}</span><span className="ml-1">courts</span></>
              )}
            </div>
          </div>
          ...
          {filtersOpen && (
            <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-pm-border/60">
              <select value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)} className="px-3 py-2 text-sm border border-pm-border rounded-xl bg-white text-pm-text focus:outline-none focus:border-pm-accent">
              ...
              <div className="flex rounded-xl border border-pm-border overflow-hidden text-sm">
                {(['all', 'indoor', 'outdoor'] as const).map(opt => (
                  <button key={opt} onClick={() => setIndoorFilter(opt)} className={`px-3 py-2 capitalize transition-colors ${indoorFilter === opt ? 'bg-pm-text text-white' : 'bg-white text-pm-muted hover:bg-pm-bg-hover'}`}>
                    {opt === 'all' ? 'All' : opt}
                  </button>
                ))}
              </div>
              <div className="flex rounded-xl border border-pm-border overflow-hidden text-sm">
                {(['all', 'venue', 'coach'] as const).map(opt => (
                  <button key={opt} onClick={() => setTypeFilter(opt)} className={`px-3 py-2 capitalize transition-colors ${typeFilter === opt ? 'bg-pm-text text-white' : 'bg-white text-pm-muted hover:bg-pm-bg-hover'}`}>
                    {opt === 'all' ? 'All' : opt === 'venue' ? 'Courts' : 'Coaches'}
                  </button>
                ))}
              </div>
              {hasActiveFilters && (
                <button onClick={clearFilters} className="text-xs text-pm-accent hover:text-pm-text transition-colors">Clear all</button>
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          <div
            ref={listRef}
            className={`w-full sm:w-[360px] lg:w-[400px] overflow-y-auto overflow-x-hidden bg-white border-r border-pm-border shrink-0 ${view === 'list' ? 'block' : 'hidden sm:block'}`}
          >
            <div className="sm:hidden px-4 py-2 bg-pm-bg border-b border-pm-border/60 text-xs text-pm-faint">
            ...
            {filtered.length === 0 ? (
              <div className="p-10 text-center">
                <p className="text-pm-muted text-sm">No venues found</p>
                <p className="text-pm-faint text-xs mt-1">Try a different search or filter</p>
                {hasActiveFilters && <button onClick={clearFilters} className="mt-3 text-xs text-pm-accent hover:underline">Clear filters</button>}
              </div>
            ) : (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      <h3 className="text-sm font-semibold text-pm-text leading-tight truncate">{listing.name}</h3>
                      <p className="text-xs text-pm-muted mt-0.5 truncate">
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      {listing.courts != null && listing.courts > 0 && (
                        <span className="text-[11px] text-pm-muted bg-pm-bg px-2 py-0.5 rounded-md border border-pm-border/40 whitespace-nowrap">
                          {listing.courts} court{listing.courts !== 1 ? 's' : ''}
                        </span>
                      )}
                      {listing.indoor !== null && (
                        <span className={`text-[10px] px-2 py-0.5 rounded-md font-medium whitespace-nowrap ${
                          listing.indoor ? 'text-emerald-700 bg-emerald-50 border border-emerald-100' : 'text-sky-700 bg-sky-50 border border-sky-100'
                        }`}>
          ...
          <div className="sm:hidden absolute bottom-6 left-1/2 -translate-x-1/2 z-30">
            ...
            >
              {view === 'list' ? (
                <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>Show Map</>
              ) : (
                <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>Show List</>
              )}
            </button>
          </div>
        </div>
